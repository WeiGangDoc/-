...
if(sgp30_get_serial_id(ID) < 0)printf(" sgp30 read serial id failed\r\n");
else
{
...
do{
	if(sgp30_read(&co2, &voc) < 0) printf("\r\n spg30 read failed");
	else printf(".");
	delay_ms(500);
}while(voc == 0 && co2 == 400);
printf("\r\n");

if(sgp30_read(&co2, &voc) < 0)printf(" sgp30 read fail\r\n");
else printf(" CO2:%dppm TVOC:%dppb\r\n", co2, voc);
